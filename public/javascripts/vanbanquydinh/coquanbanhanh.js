$(document).ready(function() {
    ...
    $('#cuocthiForm').on('submit', function(e) {
        e.preventDefault();
        let coquanbanhanh = $('#ten').val().trim();
        $.ajax({
            url: '/coquanbanhanh/add',
            method: 'POST',
            data: { coquanbanhanh },
            error: function() {
    ...
    const getCuocthiList = () => {
        table = $('#ketquaTable').DataTable({
            ...
            "aoColumns": [{
                ...
                {
                    "mData": function(data, type, dataToSet) {
                        return `<span class="ten" id="${data._id}">${data.coquanbanhanh}</span>`
                    },
                    "width": "93%"
                },
                {
    ...
    $('#EditCuocthiForm').on('submit', function(e) {
        e.preventDefault();
        let coquanbanhanh = $('#tenEdit').val().trim();
        $.ajax({
            url: '/coquanbanhanh/edit/' + idCuocthiEdit,
            method: 'POST',
            data: { coquanbanhanh },
            error: function() {
    ...
    $(document).on('click', '.deleteCuocthi', function() {
        ...
        if (isDelete) {
            $.ajax({
                url: '/coquanbanhanh/delete/' + idCuocthiDelete,
                method: 'GET',
                error: function() {
